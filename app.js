...
function FreedomGapSection() {
  ...
  const fmtC = (v) => v >= 1e6 ? "$" + (v / 1e6).toFixed(2) + "M" : v >= 1e3 ? "$" + Math.round(v / 1e3) + "K" : "$" + Math.round(v);
  const fmtTick = (v) => v >= 1e6 ? "$" + parseFloat((v / 1e6).toFixed(1)) + "M" : "$" + Math.round(v / 1e3) + "K";
